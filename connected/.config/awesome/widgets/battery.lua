...
local function update_widget(state, value)
    ...
    elseif state == 'Discharging' then
        icon:set_markup_silently(
            '<span foreground="' .. fg .. '" background="' .. bg .. '">' .. icon_discharging .. '</span>'
        )
    elseif state == 'Charging' then
        icon:set_markup_silently(
            '<span foreground="' .. fg .. '" background="' .. bg .. '">' .. icon_charging .. '</span>'
        )
...
awful.widget.watch(os.getenv('HOME') .. '/.config/awesome/widgets/battery.sh', 10, function(widget, stdout)
