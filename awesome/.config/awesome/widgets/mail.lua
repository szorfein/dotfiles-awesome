...
-- local str
local email_text_read_icon = '<span foreground="' .. fg_read .. '" background="' .. bg .. '">' .. read_icon .. '</span>'
local email_text_unread_icon = '<span foreground="'
    .. fg_unread
    .. '" background="'
    .. bg
    .. '">'
...
awful.widget.watch(
    os.getenv('HOME') .. '/.config/awesome/widgets/email.sh get',
    300, -- 5m
    function(widget, stdout, stderr, exitreason, exitcode)
        ...
        if unread_email_num > 0 then
            ...
            text:set_markup_silently('<span background="' .. bg .. '">' .. stdout .. '</span>')
...
function show_emails()
    awful.spawn.easy_async(
        os.getenv('HOME') .. '/.config/awesome/widgets/email.sh show',
...
email_widget:connect_signal('mouse::enter', function()
    show_emails()
end)
