...
for i = 1, ntags do
    ...
    tag_text[i]:buttons(gears.table.join(
        ...
        awful.button({}, 4, function()
            awful.tag.viewnext(s.tags[i].screen)
        end),
        awful.button({}, 5, function()
            awful.tag.viewprev(s.tags[i].screen)
        end)
    ))
...
local function update_widget()
    for i = 1, ntags do
        ...
        if s.tags[i] == s.selected_tag then
            tag_text[i].markup =
                helpers.colorize_text(beautiful.taglist_text_focused[i], beautiful.taglist_text_color_focused[i])
        elseif awful.tag.getproperty(s.tags[i], 'urgent') then
            tag_text[i].markup =
                helpers.colorize_text(beautiful.taglist_text_urgent[i], beautiful.taglist_text_color_urgent[i])
        elseif tag_clients and #tag_clients > 0 then
            tag_text[i].markup =
                helpers.colorize_text(beautiful.taglist_text_occupied[i], beautiful.taglist_text_color_occupied[i])
        else
            tag_text[i].markup =
                helpers.colorize_text(beautiful.taglist_text_empty[i], beautiful.taglist_text_color_empty[i])
        end
    end
end
