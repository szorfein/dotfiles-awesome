...
awful.widget.watch(os.getenv('HOME') .. '/.config/awesome/widgets/network.sh net', 60, function(_, stdout, _, _, _)
    if stdout == '1' then
        icon:set_markup_silently(
            '<span foreground="' .. fg_error .. '" background="' .. bg .. '">' .. net_icon .. '</span>'
        )
